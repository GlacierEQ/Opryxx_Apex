...
class ProjectScanner:
    ...
    def _check_architecture(self):
        ...
        if architecture_score >= 5:
            self.recommendations.append("✅ Good architectural separation")
        else:
            self.issues.append("❌ Poor architectural separation")
    ...
    def _evaluate_performance(self):
        ...
        for feature, filename in performance_features:
            if any(self.project_path.rglob(filename)):
                self.recommendations.append(f"✅ {feature} implemented")
            else:
                self.issues.append(f"❌ Missing {feature}")
    
    def _assess_ai_capabilities(self):
        """Assess AI and intelligence features"""
        print("🧠 Assessing AI capabilities...")
        ...
        if found_ai:
            self.recommendations.append("✅ AI/ML libraries detected")
        else:
            self.issues.append("❌ Limited AI capabilities")
    
    def _review_automation(self):
        """Review automation capabilities"""
        print("🤖 Reviewing automation...")
        ...
        if automation_score >= 2:
            self.recommendations.append("✅ Good automation coverage")
        else:
            self.issues.append("❌ Limited automation")
    
    def _check_gpu_npu_priority(self):
        """Check GPU/NPU prioritization"""
        print("🎮 Checking GPU/NPU priority...")
        
        gpu_files = list(self.project_path.rglob("*gpu*.py"))
        npu_files = list(self.project_path.rglob("*npu*.py"))
        
        if gpu_files:
            self.recommendations.append(f"✅ GPU support: {len(gpu_files)} files")
        else:
            self.issues.append("❌ No GPU-specific files")
            
        if npu_files:
            self.recommendations.append(f"✅ NPU support: {len(npu_files)} files")
        else:
            self.issues.append("❌ No NPU-specific files")
    ...
    def _get_priority_actions(self) -> List[str]:
        ...
        if "No GPU-specific files" in str(self.issues):
            actions.append("⚡ HIGH: Implement GPU acceleration")
            
        if "Limited AI capabilities" in str(self.issues):
            actions.append("🧠 HIGH: Add AI/ML capabilities")
            
        if "Limited automation" in str(self.issues):
...
def main():
    ...
    print(f"⚙️ Functions: {metrics['functions']}")
